...
(t/defalias TransactionData
  (t/HMap))
(t/defalias EntityTransactionData
  (t/HMap :mandatory {:db/tempid t/Number}))

(defmacro transaction! [conn-ctx bds & body])

(defn build-new [spec sp]
  "returns transaction data to build a nested datomic-data
   datastructure for the sp data"
  (check-complete! spec sp)
  (let [eid (db/tempid :db.part/user)]
    (->> (for [{iname :name [cardinality type] :type :as item} (:items spec)
               :when (iname sp)]
           (let [sub-spec (get-spec type)]
             [(db-keyword spec iname) (iname sp)]))
         (into {})
         (#(assoc % spark-type-attr (:name spec)))
         (#(assoc % :db/tempid eid)))))

(t/ann new [SpecName t/Map -> TransactionData])
(defn new 
  "takes a spec and any data structure that can be coerced into an
   instance of that spec. returns txn-data requesting a new entity be
   added to the database, including its new db-id"
  [spec-name atmap]
  (build-new (get-spec spec-name) atmap))

(defn cas [eid kw old & [new]]
  [[:db.fn/cas eid kw old (if (some? new) new old)]])

(defn entity-cas [entity old & [new]]
  (let [eid (:db/tempid entity)
        new (if (some? new) new old)]))

(t/ann add [(t/U t/Map TransactionData) -> TransactionData])
(defn add 
  ([entity] {:ops [entity]})
  ([spec-name atmap] {:ops [(build-new (get-spec spec-name) atmap)]}))

(t/ann add [(t/U t/Map TransactionData) -> TransactionData])
(defn update [atmap & kvs])

(t/ann ensure-one [(t/U t/Map TransactionData) -> TransactionData])
(defn ensure-one [entity atmap])

(t/ann retract [(t/U t/Map TransactionData) -> TransactionData])
(defn retract [entity & [attrs]]
  (let [eid   (:db/tempid entity)
        spec  (some-> entity spark-type-attr get-spec)]
    (when-not spec
     (throw (ex-info "entity to be removed has no spec" {:entity entity})))
    (if attrs
      (for [kw attrs]
        (let [db-kw (db-keyword spec kw)]
          [:db.fn/retract eid db-kw (db-kw entity)]))
...
(t/ann batch [TransactionData ... -> TransactionData])
(defn batch [& txn-data])
